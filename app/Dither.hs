...
rgb :: PixelRGBA8 -> (Pixel8,Pixel8,Pixel8)
rgb (PixelRGBA8 r g b _) = (r,g,b)
...
thresh :: (RealFrac a, Floating a, Ord a) => [[a]] -> Image PixelRGBA8 -> Int -> Int -> PixelRGBA8
thresh bmat img x y =
    let
        mh = length bmat
        mw = length $ bmat!!0
        ...
        px = pixelAt img x y
        light = lightness px
        adj = (*255) . floor $ light + bmat!!my!!mx
    in
        PixelRGBA8 adj adj adj $ alpha px

dither' :: (RealFrac a, Floating a, Ord a) => [[a]] -> Image PixelRGBA8 -> Image PixelRGBA8
